<template>
  <div style="padding: 3px">
    <vue-snotify></vue-snotify>
        ...
        <form action="#" @submit.prevent="formSubmitUpdate" method="POST">
...
export default {
  mounted(){
    this.id = this.p_id;
    this.nome = this.p_nome;
    this.cpf = this.p_cpf;
    this.telefone = this.p_telefone;
  },
  data(){
        return {
            id: '',
  ...
  methods: {
    formSubmitUpdate: function(e){
        ...
        if (this.nome && this.nome.length > qnt_chars_nome && this.cpf && this.cpf.length == qnt_chars_cpf && this.telefone) {
            this.loading = true;
            this.$store.dispatch('updatePaciente', this._data)
                        .then((e) => {
  ...
  props: ['p_nome', 'p_cpf', 'p_telefone', 'p_id']
}
</script>
